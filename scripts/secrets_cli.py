...
def _materialize(args: argparse.Namespace) -> None:
    ...
    result = _run_sops(cmd, capture_output=True)
    ...
    try:
        ...
        print(f"无法解析解密后的 JSON：{exc}", file=sys.stderr)
        raise SystemExit(1) from exc

    formatted = json.dumps(parsed, ensure_ascii=False, indent=2) + "\n"
    target.write_text(formatted, encoding="utf-8")
    target.chmod(stat.S_IRUSR | stat.S_IWUSR)
    print(f"已生成解密 JSON: {target}")
...
def _parser() -> argparse.ArgumentParser:
    ...
    materialize = sub.add_parser("materialize", help="解密密文并写入 JSON 以供 Nix 评估")
    materialize.add_argument("--source", default="secrets/cfg.secrets.yaml", help="密文路径")
    materialize.add_argument(
        "--target",
        default=".direnv/secrets/cfg.secrets.json",
        help="输出的 JSON 文件",
    )
