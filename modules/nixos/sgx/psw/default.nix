...
stdenv.mkDerivation rec {
  ...
  patches = [
    ...
    # Make sgx_edger8r use explicit stdlib search paths under OCaml 5.
    ./ocaml-5-include-paths.patch

    # Ensure sgx_enclave_common builds with newer libstdc++ by pulling in <algorithm>/<vector>.
    ./sgx_enclave_common-add-includes.patch

    # Bump bundled CppMicroServices to require CMake >= 3.5 for compatibility with modern CMake.
    ./cppmicroservices-cmake-min-3.5.patch

    # Raise AESM service cmake requirement to match modern CMake defaults.
    ./aesm-cmake-3.5-cxx20.patch
  ];
  ...
  postPatch =
    ...
    ''
      ...
      # Update edger8r sources for OCaml 5 compatibility.
      sed -i 's/let failwithf fmt = kprintf failwith fmt;;/let failwithf fmt = ksprintf failwith fmt;;/' \
